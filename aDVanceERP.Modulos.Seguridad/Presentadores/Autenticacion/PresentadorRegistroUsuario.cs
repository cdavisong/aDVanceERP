    ...
    protected override Core.Modelos.Modulos.Seguridad.CuentaUsuario? ObtenerEntidadDesdeVista() {
        ...
        try {
            ...
            if (repoCuentaUsuario.Cantidad() == 0) {
                var rolAdministrador = repoRolUsuario.Buscar(FiltroBusquedaRolUsuario.Nombre, "Administrador").entidades.FirstOrDefault();

                if (rolAdministrador != null) 
                    usuario.IdRolUsuario = rolAdministrador.Id;
                else { 
                    rolAdministrador = new Core.Modelos.Modulos.Seguridad.RolUsuario(0, "Administrador"); 
